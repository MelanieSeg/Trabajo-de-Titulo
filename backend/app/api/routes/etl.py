...
from app.services.etl_service import run_etl_from_csv, run_etl_from_fuel_csv, run_etl_from_resource_csv, run_etl_from_utility_csv
...
@router.post("/upload/fuel-transactions", response_model=ETLUploadResponse)
def upload_fuel_transactions_csv(
    file: UploadFile = File(...),
    db: Session = Depends(get_db_session),
) -> ETLUploadResponse:
    filename = _validate_csv_filename(file.filename)
    ...
    try:
        file_path.write_bytes(file.file.read())
        job = run_etl_from_fuel_csv(db, csv_path=str(file_path), source_filename=filename)
        db.commit()
    except Exception as exc:
        db.rollback()
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
@router.post("/upload/resource/{code}", response_model=ETLUploadResponse)
def upload_resource_csv(
    code: str,
    file: UploadFile = File(...),
    db: Session = Depends(get_db_session),
) -> ETLUploadResponse:
    ...
    try:
        file_path.write_bytes(file.file.read())
        job = run_etl_from_resource_csv(db, code=code, csv_path=str(file_path), source_filename=filename)
        db.commit()
    except Exception as exc:
        db.rollback()
        raise HTTPException(status_code=400, detail=str(exc)) from exc
